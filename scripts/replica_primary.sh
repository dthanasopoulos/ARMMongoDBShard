...
do
	let a=$i+$g
	mongo -u "$mongoAdminUser" -p "$mongoAdminPasswd" "admin" --eval "printjson(rs.add('10.100.6.${a}:27017'))"
	if [[ $? -eq 0 ]];then
		echo "adding server 10.100.6.${a} successfully"
	else
		echo "adding server 10.100.6.${a} failed!"
	fi
done
